...
public class BaseDataServiceImpl implements BaseDataService {
	...
	public ResultUtil getBaseData(String dataName, int pageIndex, int pageSize, String criteria, int sort) {
		...
		if (sort == 0){
			sql= "select distinct id,data_type as \"dataType\",data_name as \"dataName\",value_1 as \"value1\",value_2 as \"value2\" " +
					"from base_data where data_name like '%"+dataName+"%' order by "+criteria+" ";
		}
		else {
			sql= "select distinct id,data_type as \"dataType\",data_name as \"dataName\",value_1 as \"value1\",value_2 as \"value2\" " +
					"from base_data where data_name like '%"+dataName+"%' order by "+criteria+" desc ";
		}
	...
	public ResultUtil addBaseData(List<?> list) {
		...
		String sql = "insert into base_data (data_type, data_name, value_1, value_2) values"+stringBuilder.toString()+";";
	...
	public ResultUtil deleteBaseData(List<?> list) {
		...
		sql = "delete from base_data where id in ("+deleteBuild.toString()+");";
	...
	public ResultUtil getWharf() {
		String sql= "select value_1 as \"wharf\" from  base_data where data_type = 4 ";
		String message;
	...
	public ResultUtil updateBaseData(List<?> list) {
		...
		for (int i = 0; i <list.size() ; i++) {
			...
			if(!isBaseDataExist){
				...
				sql = "update base_data set data_type= "+dataType.orElse("-1")+",data_name = '"+dataName+"',value_1= '"+value1.orElse("")+"',value_2 = '"+value2.orElse("")+"' where id= "+id.orElse("-1")+" ;";
	...
	public boolean checkBaseDataExist(int id){
		String message;
		String sql = "select 1 from base_data where id="+id+";";
		ResultUtil result=dbHelperService.select(sql,"postgres_test");
		if(HttpStatus.OK.value()!= (int)result.get("code")){
			message=MessageUtil.getMessage(Message.QUERY_ERROR.getCode());
			logger.error(message);
	...
	public boolean checkBaseDataExist(String dataType,String value1,String value2){
		String message;
		String sql = "select 1 from base_data where data_type='"+dataType+"' and value_1='"+value1+"' and value_2='"+value2+"';";
		ResultUtil result=dbHelperService.select(sql,"postgres_test");
		if(HttpStatus.OK.value()!= (int)result.get("code")){
			message=MessageUtil.getMessage(Message.QUERY_ERROR.getCode());
			logger.error(message);
	...
	public int getLoadTimeOut(){
		String sql= "select  value_1  as \"loadTimeOut\" from  base_data where data_type = 5 order by id desc limit 1 ";
		String message;
		ResultUtil result=dbHelperService.select(sql,"postgres_test");
		if(HttpStatus.OK.value()!= (int)result.get("code")){
			message=MessageUtil.getMessage(Message.QUERY_ERROR.getCode());
	...
	public int getWaitTimeOut(){
		String sql= "select  value_1  as \"waitTimeOut\" from  base_data where data_type = 6 order by id desc limit 1 ";
		String message;
		ResultUtil result=dbHelperService.select(sql,"postgres_test");
		if(HttpStatus.OK.value()!= (int)result.get("code")){
			message=MessageUtil.getMessage(Message.QUERY_ERROR.getCode());
